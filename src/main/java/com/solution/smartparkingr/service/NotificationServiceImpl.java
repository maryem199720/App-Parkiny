...
public class NotificationServiceImpl implements NotificationService {
    ...
    public void checkUpcomingEvents() {
        ...
        // Reservations about to start (within 15 minutes)
        List<Reservation> upcomingReservations = reservationRepository.findUpcomingReservationsByStartTime(now, fifteenMinFromNow);
        for (Reservation res : upcomingReservations) {
            // Check if notification was already sent within the last 15 minutes
            boolean alreadyNotified = notificationRepository.findByUserIdOrderByCreatedAtDesc(res.getUser().getId())
                    .stream()
                    .anyMatch(n -> n.getType() == Notification.NotificationType.RESERVATION_START &&
                            n.getMessage().contains("ID: " + res.getId()) &&
                            n.getCreatedAt().isAfter(now.minusMinutes(15)));
            if (!alreadyNotified) {
        ...
        // Reservations about to end (within 5 minutes)
        ...
        for (Reservation res : endingReservations) {
            // Check if notification was already sent within the last 5 minutes
            boolean alreadyNotified = notificationRepository.findByUserIdOrderByCreatedAtDesc(res.getUser().getId())
                    .stream()
                    .anyMatch(n -> n.getType() == Notification.NotificationType.RESERVATION_END &&
                            n.getMessage().contains("ID: " + res.getId()) &&
                            n.getCreatedAt().isAfter(now.minusMinutes(5)));
            if (!alreadyNotified) {
                String message = String.format("Your reservation (ID: %d) ends at %s.", res.getId(), res.getEndTime());
                sendNotification(res.getUser().getId(), Notification.NotificationType.RESERVATION_END, message, null);
            }
        }

        // Subscriptions about to expire (within 3 days)
        ...
        for (Subscription sub : expiringSubscriptions) {
            // Check if notification was already sent within the last 3 days
            boolean alreadyNotified = notificationRepository.findByUserIdOrderByCreatedAtDesc(sub.getUser().getId())
                    .stream()
                    .anyMatch(n -> n.getType() == Notification.NotificationType.SUBSCRIPTION_RENEWAL &&
                            n.getMessage().contains("ID: " + sub.getId()) &&
                            n.getCreatedAt().isAfter(now.minusDays(3)));
            if (!alreadyNotified) {
